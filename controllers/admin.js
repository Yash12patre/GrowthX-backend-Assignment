import TASK from "../model/TASK.js"; // Ensure the path is correct and ends with .js
...
const getTaggedAssignments = async (req, resp) => {
  ...
  try {
    const taggedAssignment = await TASK.find({
      admin: req.user._id,
      status: "Pending",
    })
      .populate("userId", "name email")
      .select("-password");
    if (taggedAssignment.length === 0) {
      return resp.status(404).json({ error: "No tagged Assignments found" });
    } else {
      return resp.status(200).json(taggedAssignment);
    }
  } catch (error) {
    console.log(error);
